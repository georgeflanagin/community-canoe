...
class FIFO:
    ...
    def __init__(self, 
        ...
        try:
            ...
            if stat.S_ISFIFO(os.stat(self.name).st_mode):
                ...
                uu.tombstone(f"{self.name} is reopened")
                return
        ...
        try:
            ...
            uu.tombstone(f"created new FIFO {self.name}")
            self.fifo = os.open(self.name, FIFO.modes[self.mode])
            uu.tombstone(f"{self.name} is open in mode <{self.mode}>")
    ...
    def write(self, messages:List[str]) -> int:
        ...
        messages = self.delimiter.join(uu.listify(messages))
        try:
