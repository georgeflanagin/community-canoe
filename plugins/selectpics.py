...
@trap
def selectpics_main(opcodes:list) -> ERROR_ACTION:
    ...
    for i, subroutine in enumerate(opcodes):
        ...
        frame = p.read('pandas')
        photo_IDs = frame[subroutine.columns]
        for photo_ID in photo_IDs:
            try:
                os.link(uu.path_join(subroutine.directory, photo_ID), 
                        uu.path_join(subroutine.local_dir, photo_ID))
            except Exception as e:
