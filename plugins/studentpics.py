# -*- coding: utf-8 -*-
""" 
Make links to the student pictures.
"""
...
@trap
def studentpics_main(opcodes:list) -> ERROR_ACTION:
    ...
    for i, subroutine in enumerate(uu.listify(opcodes)):
        ...
        command = f"/usr/bin/link {opcodes.images}/{{}}.jpg {opcodes.local_dir}/{{}}.jpg"
        p = urpacker.URpacker()
        if not p.attachIO(subroutine.input, s_mode='read'):
            tomb.tombstone(f'{subroutine.input} could not be found/opened.')
            stats.update(myname, mytype, LED.RED)
            return subroutine.on_error
        
        filenames = p.read('pandas')[subroutine.column]
        for f in filenames:
            uu.dorunrun(command.format(f, f))
