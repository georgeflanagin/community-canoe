...
@trap
def bunzip2_main(opcodes:list) -> ERROR_ACTION:
    ...
    for i, subroutine in enumerate(uu.listify(opcodes)):
        ...
        p = urpacker.URpacker()
        p.attachIO(os.path.join(subroutine.local_dir, subroutine.input), s_mode='read')
        data = p.read('pandas')
        data.to_msgpack()
        with open(os.path.join(subroutine.local_dir, subroutine.output), 'wb') as f:
            f.write(data)
